...
#include "estimator.h"
#include "estimator_eso.h" // changed
#include "usddeck.h"
...
// ESO
static eso_t esoData;
static control_t controlPrev; // changed
...
static void stabilizerTask(void* param)
{
  ...
  while(1) {
    ...
    if (healthShallWeRunTest()) {
      ...
      esoUpdate(&esoData, &state, &sensorData, &controlPrev); // changed
      ...
      esoApplyDisturbanceCompensation(&control, &esoData); // changed
      
      controlPrev = control; // changed
      // Critical for safety, be careful if you modify this code!
...
extern eso_log_int_t esoLogInt;   // compact INT32 log struct

LOG_GROUP_START(eso)
LOG_ADD(LOG_INT32, x, &esoLogInt.x)
LOG_ADD(LOG_INT32, y, &esoLogInt.y)
LOG_ADD(LOG_INT32, z, &esoLogInt.z)
LOG_ADD(LOG_INT32, t, &esoLogInt.t)
LOG_GROUP_STOP(eso)
