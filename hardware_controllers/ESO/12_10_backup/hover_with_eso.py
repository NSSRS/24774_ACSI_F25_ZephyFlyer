...
def plot_results(H):
    ...
    y = np.array(H['eso_y'])
    z = np.array(H['eso_z'])
    Fx = np.array(H['dist_x'])
    Fy = np.array(H['dist_y'])
    Fz = np.array(H['dist_z'])

    # ---- 3D Plot Only ----
    fig = plt.figure(figsize=(10, 9))
    ax = fig.add_subplot(111, projection="3d")
    ax.set_title("3D ESO Trajectory + Force Vectors")

    ax.plot3D(x, y, z, "b", label="ESO Trajectory")

    # Downsample
    step = max(5, len(x)//100)
    ax.quiver(
...
# =============================================================
# =============================================================
# HOVER ROUTINE (only hold at 0.5 m)
# =============================================================
def fly_circle(scf, radius=RADIUS, height=HEIGHT, circle_time=CIRCLE_TIME, filename=None):
    ...
    try:
        with MotionCommander(scf, default_height=height) as mc:
            ...
            print("Holding hover at 0.5 m...")

            HOVER_TIME = 8.0   # seconds you want to hover
